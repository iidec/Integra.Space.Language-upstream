...
namespace Integra.Space.Language
{
    ...
    internal sealed class AlterDatabaseNode : AlterObjectNode<DatabaseOptionEnum>
    {
        ...
        public AlterDatabaseNode(CommandObject commandObject, Dictionary<DatabaseOptionEnum, object> options, int line, int column, string nodeText) : base(commandObject, options, line, column, nodeText, null, null)
        {
            if (options.ContainsKey(DatabaseOptionEnum.Name))
            {
                ...
                this.CommandObjects.Add(new CommandObject(SystemObjectEnum.Database, options[DatabaseOptionEnum.Name].ToString(), PermissionsEnum.None, true));
            }
        }
    }
}
