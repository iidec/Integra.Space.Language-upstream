    ...
    internal sealed class TreeTransformations
    {
        ...
        private void InsertGenerationOfLocalObjects(IEnumerable<IGrouping<string, PlanNode>> objects)
        {
            ...
            foreach (IGrouping<string, PlanNode> @object in objects)
            {
                ...
                IEnumerable<PlanNode> replicas = this.CreateACopyOfQueryObjectsAccessors(@object.Where(x => (x.NodeType == PlanNodeTypeEnum.EventProperty && !x.Properties["Property"].ToString().Equals("SystemTimestamp"))).Distinct(new PropertyNameComparer()));
        ...
        private IEnumerable<IGrouping<string, PlanNode>> GetValuesFromEvents(PlanNode branch)
        {
            ...
            IEnumerable<IGrouping<string, PlanNode>> objects = branch.FindNode(PlanNodeTypeEnum.Property)
                .GroupBy(x => x.FindNode(PlanNodeTypeEnum.ObservableFromForLambda).Single().Properties["SourceName"].ToString());

            /* 
            objects = branch.FindNode(PlanNodeTypeEnum.ObjectValue, PlanNodeTypeEnum.EventProperty)
                        .GroupBy(x =>
                        {
        ...
        private void AddSourceIdToEventNodes()
        {
            ...
            List<PlanNode> sourceRefNodes = this.executionPlanRootNode.FindNode(PlanNodeTypeEnum.Property).Select(x => x.Children.Single()).ToList(); // new List<PlanNode>(); // this.executionPlanRootNode.FindNode(PlanNodeTypeEnum.Event);
